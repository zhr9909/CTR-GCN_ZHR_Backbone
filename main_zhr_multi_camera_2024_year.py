...
from model.ctrgcn_zhr_2024 import Temporal_CNN_Model
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data_1, label_1, index, data_2, label_2, index_2) in enumerate(process):
            ...
            # print(similarity_numpy[0:64])

            normsSampleFeature_1 = torch.norm(x_tensor_1, dim=1, keepdim=True)
            normalized_sampleFeature_1 = x_tensor_1 / normsSampleFeature_1
            normsSampleFeature_2 = torch.norm(x_tensor_2, dim=1, keepdim=True)
            normalized_sampleFeature_2 = x_tensor_2 / normsSampleFeature_2
            similarity_numpy_matrix = torch.matmul(normalized_sampleFeature_1, normalized_sampleFeature_2.T)
            # print(similarity_numpy_matrix[0])
            ...
            if batch_idx % 200 == 0:
                ...
                torch.save(weights, 'data/ntu/checkpoints_4/'+'runs' + '-' + 'main_zhr_'+str(epoch+1) + '-' + str(int(self.global_step)) + '.pt')
                # 保存模型
                # break
