...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    if num_frames_drop >= num_frames:
        try:
            with open('raw_data_zhr/error_skes.txt', 'a') as txt_file:
...
def get_raw_skes_data():
    ...
    np.savetxt(osp.join(save_path, 'raw_data_zhr', 'frames_cnt.txt'), frames_cnt, fmt='%d')
...
if __name__ == '__main__':
    ...
    stat_path = osp.join(save_path, 'statistics_zhr')
    if not osp.exists('./raw_data_zhr'):
        os.makedirs('./raw_data_zhr')

    error_skes = 'raw_data_zhr/error_skes.txt'

    skes_name_file = osp.join(stat_path, 'skes_available_name.txt')
    save_data_pkl = osp.join(save_path, 'raw_data_zhr', 'raw_skes_data.pkl')
    frames_drop_pkl = osp.join(save_path, 'raw_data_zhr', 'frames_drop_skes.pkl')

    frames_drop_logger = logging.getLogger('frames_drop')
    frames_drop_logger.setLevel(logging.INFO)
    frames_drop_logger.addHandler(logging.FileHandler(osp.join(save_path, 'raw_data_zhr', 'frames_drop.log')))
    frames_drop_skes = dict()
