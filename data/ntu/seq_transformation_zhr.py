...
root_path = './'
stat_path = osp.join(root_path, 'statistics_zhr_2')
...
denoised_path = osp.join(root_path, 'denoised_data_zhr')
...
def split_dataset_zhr(skes_joints, label, performer, camera, evaluation, save_path):
    
    camera1_indices, camera2_indices, camera3_indices = get_indices_zhr(performer, camera, evaluation)
    m = 'sklearn'  # 'sklearn' or 'numpy'

    camera1_labels = label[camera1_indices]
    camera2_labels = label[camera2_indices]
    camera3_labels = label[camera3_indices]

    camera1_x = skes_joints[camera1_indices]
    camera1_y = one_hot_vector(camera1_labels)
    camera2_x = skes_joints[camera2_indices]
    camera2_y = one_hot_vector(camera2_labels)
    camera3_x = skes_joints[camera3_indices]
    camera3_y = one_hot_vector(camera3_labels)

    save_name = 'NTU60_zhr2.npz'
    np.savez(save_name, camera1_x=camera1_x, camera1_y=camera1_y, camera2_x=camera2_x, camera2_y=camera2_y, camera3_x=camera3_x, camera3_y=camera3_y)

def get_indices_zhr(performer, camera, evaluation='zhr'):
    camera1_indices = np.empty(0)
    camera2_indices = np.empty(0)
    camera3_indices = np.empty(0)

    if evaluation == 'zhr':
        # Get indices of test data
        temp = np.where(camera == 1)[0]  # 0-based index
        camera1_indices = np.hstack((camera1_indices, temp)).astype(np.int)

        temp = np.where(camera == 2)[0]  # 0-based index
        camera2_indices = np.hstack((camera2_indices, temp)).astype(np.int)

        temp = np.where(camera == 3)[0]  # 0-based index
        camera3_indices = np.hstack((camera3_indices, temp)).astype(np.int)


    return camera1_indices,camera2_indices,camera3_indices
...
if __name__ == '__main__':
    ...
    for evaluation in evaluations:
        print('小z')
        split_dataset_zhr(skes_joints, label, performer, camera, evaluation, save_path)
